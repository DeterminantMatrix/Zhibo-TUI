...
from .bilibili_quality import bilibili_quality_qn, fetch_best_bilibili_stream
...
STREAM_URL_TIMEOUT = 10


def _streamget_error(exc: Exception) -> str:
    text = " ".join(str(exc).split())
    if "play_url_list" in text and "unexpected keyword argument" in text:
        return "streamget 与当前平台响应结构不兼容（play_url_list）"
    return text[:500]
...
class StreamgetPlugin(LiveStreamPlugin):
    ...
    async def check_live(self, url: str, **kwargs) -> LiveInfo:
        ...
        try:
            ...
            return LiveInfo(is_live=False, extra={"error": _streamget_error(e), "platform": get_display_name(platform_key), "proxy": bool(proxy_url)})
        ...
        try:
            ...
            stream_error = "获取流地址超时"
        except Exception as e:
            stream_error = _streamget_error(e)
        ...
        if stream_obj is None:
            return LiveInfo(
                is_live=False,
                extra={
                    "error": stream_error or "无法获取流地址",
                    "platform": get_display_name(platform_key),
        ...
        if stream_obj.is_live and bilibili_quality_qn(platform_key, quality) is not None:
            try:
                ...
                if best_bili and best_bili.get("url"):
                    ...
                    extra["stream_candidates"] = list(best_bili.get("candidates") or [])
